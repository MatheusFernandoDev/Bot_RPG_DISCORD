...
nomes = pd.read_csv("bot/arquivos/csv/nomes_escocesos.csv")
...
def merc_raposaceo():
    ...
    if(escolha == 1):
        atributos1["eva"] += random.randint(1, 3)
    else:
        atributos1["des"] += random.randint(1,3)
...
def player_raposaceo():
    ...
    if(escolha == 1):
        atributos["eva"] += random.randint(1, 3)
    else:
        atributos["des"] += random.randint(1,3)
